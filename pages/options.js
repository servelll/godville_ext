...
function UpdateFieldChildObject(line_obj) {
    ...
    const field_childobj =
        line.querySelector('.title_print') ??
...
document.querySelector('#seaMonsters .update').addEventListener('click', (e) => {
    ...
    const time = performance.now();
    const label = e.target.parentNode.querySelector('.correct_link');
    CorsGetPageFromUrl(label.title)
        .then((html) => {
            console.log(html);
            ...
            if (e.target.title) e.target.title += '\n';
            e.target.title += `loaded with CORS-proxy (${(performance.now() - time).toFixed(1)}ms) ${html
                .querySelector('s')
                .getAttribute('href')}`;
        ...
        .catch((et) => {
            e.target.parentNode.parentNode.querySelector('.field_content label').textContent =
                new Date().toLocaleString() + ' ' + et;
        });
});
