...
if (document.location.href.includes('https://gv.erinome.net/duels/log/')) {
    ...
    AddAbstractChromeStorageListener('Logs', (add_arr, new_arr, ch_key) => {
        ...
        add_arr.forEach((x_id) => {
            ...
            if (bool != undefined && id == x_id)
                MarkRow(a.parentNode, bool, 'https://gv.erinome.net/duels/log/' + x_id);
        });
    });
}
...
class PolygonLog {
    ...
    MatchLabelsWithSubindexes(index) {
        if (d[index].length == 4)
            return let_mas.reduce(function (accumulator, currentValue, index) {
...
if (header?.textContent == 'Полигон') {
    ...
    AddMovesCountVisibleTools('#dmap', '#hero2 .block_h');
}
